        ...
        <footer className="flex-shrink-0 w-full flex items-center justify-between px-8 py-5 border-t border-border">
            ...
            <div className="flex items-center gap-2">
                {[
                    ...
                    <a key={label} href={href} aria-label={label} className="w-8 h-8 flex items-center justify-center rounded border border-border bg-card text-muted-foreground hover:text-foreground hover:border-accent/50 transition-all">
                        <Image src={`/icons/${label.toLowerCase()}.svg`} alt={label} width={16} height={16} className="w-4 h-4 dark:invert" />
                    </a>
                ))}
            </div>
            <a href={`mailto:${PERSON.email}`} className="flex items-center gap-1.5 text-[11px] text-muted-foreground hover:text-foreground tracking-widest transition-colors">
